				...
				<div class="top_block_filter_section">
					...
					<div class="items">
						<?foreach($arAllSections as $key => $arTmpSection):?>
							<div class="item <?=($setionIDRequest ? ($key == $setionIDRequest ? 'current' : '') : '');?>"><a href="<?=$APPLICATION->GetCurPageParam('section_id='.$key, $arDeleteParams)?>" class="dark_link"><span><?=$arSections[$key]["NAME"];?></span><noindex><span><?=$arTmpSection["COUNT"];?></span></noindex></a></div>
						<?endforeach;?>
					</div>
				</div>
			<?endif;?>
