	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
		{
			...
			if (intval($propertyID) > 0)
			{
				...
				{
					$arUpdatePropertyValues[$propertyID] = array();
					foreach ($_FILES["PROPERTY_FILE_".$propertyID]["name"] as $key => $value)
					{
						$arFile["name"] = $_FILES["PROPERTY_FILE_".$propertyID]["name"][$key];
						$arFile["type"] = $_FILES["PROPERTY_FILE_".$propertyID]["type"][$key];
						$arFile["tmp_name"] = $_FILES["PROPERTY_FILE_".$propertyID]["tmp_name"][$key];
						$arFile["error"] = $_FILES["PROPERTY_FILE_".$propertyID]["error"][$key];
						$arFile["size"] = $_FILES["PROPERTY_FILE_".$propertyID]["size"][$key];
			...
			else
			{
				...
				else
				{
					if($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "F")
					{
						$arFile = $_FILES["PROPERTY_FILE_".$propertyID];
